...
describe("Pure Yul ERC1155 - safeTransferFrom Function Testing", function () {
    ...
    describe("Transfer from - Case 01", async () => {
        ...
        it("Transfer From with user2 set user1 the approve all", async () => {
            ...
            await tnx1.wait()
            const tnx2 = await contract.safeTransferFrom(user2.address, user1.address, 1, 100, 0x00)
            ...
            testTransferSingleEmitResults(3, receipt, deployer, user2, user1, 1, 100)
        ...
        it("Transfer From with user1 set user2 the approve all", async () => {
            ...
            await tnx1.wait()
            const tnx2 = await contract.safeTransferFrom(user1.address, user2.address, 2, 100, 0x00)
            const receipt = await tnx2.wait()
            ...
            testTransferSingleEmitResults(4, receipt, deployer, user1, user2, 2, 100)
