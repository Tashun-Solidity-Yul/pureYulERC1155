...
function testTransferSingleEmitResults(testId: number, receipt1: any, deployer: any, user: any, tokenId: number, tokenAmount: number) {
    ...
    assert(decodedData1)
    expect(decodedData1[0], `TestId ${testId}, Token Amount is wrong`).to.be.equal(tokenId);
    expect(decodedData1[1], `TestId ${testId}, Token Amount is wrong`).to.be.equal(tokenAmount);
}
...
describe("Pure Yul ERC1155 - Mint Function Testing", function () {
    ...
    describe("Mint Values - Case 01", async () => {
        ...
        it("Mint Multiple using multiple transactions and multiple users, tnx by multi users", async () => {
            ...
            expect(await contract.balanceOf(user1.address, 6), "Mint Multiple using multiple transactions and multiple users balance is wrong").to.be.equal(5506);
            expect(await contract.balanceOf(user2.address, 6), "Mint Multiple using multiple transactions and multiple users balance is wrong").to.be.equal(3508);
    ...
    describe("Mint Values minting to a contract - Case 03", async () => {
         ...
         **/
        it("Mint a single Token", async () => {
            const mint = await contract.mint(validContract.address, 1, 1, 0x0);
            const receipt = await mint.wait();

            expect(await contract.balanceOf(validContract.address, 1), "Balance is wrong").to.be.equal(1);

            testTransferSingleEmitResults(1, receipt, deployer, validContract, 1, 1);

        })


        it("Mint with Multiple Tokens", async () => {
            const mint = await contract.mint(validContract.address, 2, 2467, 0x0);
            const receipt = await mint.wait();

            expect(await contract.balanceOf(validContract.address, 2), "Mint Multiple Tokens Balance is wrong").to.be.equal(2467);

            testTransferSingleEmitResults(2, receipt, deployer, validContract, 2, 2467);

        })

        it("Mint with Multiple users", async () => {
            const mint1 = await contract.mint(validContract.address, 3, 2000, 0x0);
            const mint2 = await contract.mint(validContract.address, 3, 1000, 0x0);
            const receipt1 = await mint1.wait();
            const receipt2 = await mint2.wait();

            expect(await contract.balanceOf(validContract.address, 3), "Mint Multiple users Balance is wrong").to.be.equal(3000);

            testTransferSingleEmitResults(3, receipt1, deployer, validContract, 3, 2000);
            testTransferSingleEmitResults(4, receipt2, deployer, validContract, 3, 1000);

        })

        it("Mint Multiple using multiple transaction", async () => {
            const mint1 = await contract.mint(validContract.address, 4, 2000, 0x0);
            const mint2 = await contract.mint(validContract.address, 4, 1000, 0x0);
            const mint3 = await contract.mint(validContract.address, 4, 2000, 0x0);
            const mint4 = await contract.mint(validContract.address, 4, 500, 0x0);
            const mint5 = await contract.mint(validContract.address, 4, 6, 0x0);
            const receipt1 = await mint1.wait();
            const receipt2 = await mint2.wait();
            const receipt3 = await mint3.wait();
            const receipt4 = await mint4.wait();
            const receipt5 = await mint5.wait();
            expect(await contract.balanceOf(validContract.address, 4), "Mint Multiple using multiple transaction Balance is wrong").to.be.equal(5506);

            testTransferSingleEmitResults(5, receipt1, deployer, validContract, 4, 2000);
            testTransferSingleEmitResults(6, receipt2, deployer, validContract, 4, 1000);
            testTransferSingleEmitResults(7, receipt3, deployer, validContract, 4, 2000);
            testTransferSingleEmitResults(8, receipt4, deployer, validContract, 4, 500);
            testTransferSingleEmitResults(9, receipt5, deployer, validContract, 4, 6);
        })
        after(async () => {
        })

        it("Mint Multiple using multiple transactions and multiple users", async () => {
            const mint1 = await contract.mint(validContract.address, 5, 2000, 0x0);
            const mint6 = await contract.mint(user2.address, 5, 500, 0x0);
            const mint2 = await contract.mint(validContract.address, 5, 1000, 0x0);
            const mint7 = await contract.mint(user2.address, 5, 500, 0x0);
            const mint3 = await contract.mint(validContract.address, 5, 2000, 0x0);
            const mint4 = await contract.mint(validContract.address, 5, 500, 0x0);
            const mint8 = await contract.mint(user2.address, 5, 1000, 0x0);
            const mint5 = await contract.mint(validContract.address, 5, 6, 0x0);
            const mint9 = await contract.mint(user2.address, 5, 1500, 0x0);
            const mint10 = await contract.mint(user2.address, 5, 8, 0x0);
            const receipt1 = await mint1.wait();
            const receipt2 = await mint2.wait();
            ...
            expect(await contract.balanceOf(validContract.address, 5), "Mint Multiple using multiple transactions and multiple users balance is wrong").to.be.equal(5506);
            expect(await contract.balanceOf(user2.address, 5), "Mint Multiple using multiple transactions and multiple users balance is wrong").to.be.equal(3508);

            testTransferSingleEmitResults(10, receipt1, deployer, validContract, 5, 2000);
            testTransferSingleEmitResults(11, receipt2, deployer, validContract, 5, 1000);
            testTransferSingleEmitResults(12, receipt3, deployer, validContract, 5, 2000);
            testTransferSingleEmitResults(13, receipt4, deployer, validContract, 5, 500);
            testTransferSingleEmitResults(14, receipt5, deployer, validContract, 5, 6);
            testTransferSingleEmitResults(15, receipt6, deployer, user2, 5, 500);
            testTransferSingleEmitResults(16, receipt7, deployer, user2, 5, 500);
            testTransferSingleEmitResults(17, receipt8, deployer, user2, 5, 1000);
            testTransferSingleEmitResults(18, receipt9, deployer, user2, 5, 1500);
        ...
        it("Mint Multiple using multiple transactions and multiple users, tnx by multi users", async () => {
            ...
            const mint2 = await contract.connect(user2).mint(user1.address, 6, 1000, 0x0);
            const mint7 = await contract.connect(user2).mint(user2.address, 6, 500, 0x0);
            const mint3 = await contract.connect(user2).mint(validContract.address, 6, 2000, 0x0);
            const mint4 = await contract.connect(user2).mint(user1.address, 6, 500, 0x0);
            const mint8 = await contract.connect(user1).mint(user2.address, 6, 1000, 0x0);
            const mint5 = await contract.connect(user1).mint(validContract.address, 6, 6, 0x0);
            const mint9 = await contract.connect(user2).mint(user2.address, 6, 1500, 0x0);
            const mint10 = await contract.connect(user2).mint(validContract.address, 6, 8, 0x0);
            const receipt1 = await mint1.wait();
            const receipt2 = await mint2.wait();
            const receipt3 = await mint3.wait();
            const receipt4 = await mint4.wait();
            ...
            expect(await contract.balanceOf(user1.address, 6), "Mint Multiple using multiple transactions and multiple users balance is wrong").to.be.equal(3500);
            expect(await contract.balanceOf(user2.address, 6), "Mint Multiple using multiple transactions and multiple users balance is wrong").to.be.equal(3500);
            expect(await contract.balanceOf(validContract.address, 6), "Mint Multiple using multiple transactions and multiple users balance is wrong").to.be.equal(2014);

            testTransferSingleEmitResults(21, receipt2, user2, user1, 6, 1000);
            testTransferSingleEmitResults(20, receipt1, user1, user1, 6, 2000);
            testTransferSingleEmitResults(22, receipt3, user2, validContract, 6, 2000);
            testTransferSingleEmitResults(23, receipt4, user2, user1, 6, 500);
            testTransferSingleEmitResults(24, receipt5, user1, validContract, 6, 6);
            testTransferSingleEmitResults(25, receipt6, user2, user2, 6, 500);
            testTransferSingleEmitResults(26, receipt7, user2, user2, 6, 500);
            testTransferSingleEmitResults(27, receipt8, user1, user2, 6, 1000);
            testTransferSingleEmitResults(28, receipt9, user2, user2, 6, 1500);
            testTransferSingleEmitResults(29, receipt10, user2, validContract, 6, 8);
